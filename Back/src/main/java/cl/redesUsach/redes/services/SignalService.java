...
public class SignalService {
	...
	@Transactional
	public HttpStatus sendEmail() throws FileNotFoundException, DocumentException, ParseException {
		...
		File archivo = new File("/home/DIINF/ncallahan/proyectos/iTextHelloWorld.pdf");
		...
		Font fontTitulo = FontFactory.getFont(FontFactory.TIMES_ROMAN, 16, BaseColor.BLACK);
		Font fontLugar = FontFactory.getFont(FontFactory.TIMES_ROMAN,14,BaseColor.BLACK);
		Paragraph p = new Paragraph();
		p.setFont(fontTitulo);
		p.add("Reporte de estado de la red");
		...
		for (Lugar ubicacion : lugares) {
			...
			Double velocidadMañana = 0.0;
			Double intensidadMañana = 0.0;
			Double velocidadTarde = 0.0;
			Double intensidadTarde = 0.0;
			Double velocidadNoche = 0.0;
			Double intensidadNoche = 0.0;
			Double contadorM = 0.0;
			Double contadorT = 0.0;
			Double contadorN = 0.0;
			...
			for (Signal señal : señal1) {
				if (señal.getEstado() != null) {
					if (señal.getBloque().equals("Mañana")) {
						contadorM++;
						velocidadMañana = velocidadMañana + Double.parseDouble(señal.getVelocidad());
						intensidadMañana = intensidadMañana + Double.parseDouble(señal.getIntensidad());
					} else if (señal.getBloque().equals("Tarde")) {
						contadorT++;
						velocidadTarde = velocidadTarde + Double.parseDouble(señal.getVelocidad());
						intensidadTarde = intensidadTarde + Double.parseDouble(señal.getIntensidad());
					} else {
						contadorN++;
						velocidadNoche = velocidadNoche + Double.parseDouble(señal.getVelocidad());
						intensidadNoche = intensidadNoche + Double.parseDouble(señal.getIntensidad());

					}
				}
			}
			Double velocidadPromedioM = 0.0;
			Double intensidadPromedioM = 0.0;
			Double velocidadPromedioT = 0.0;
			Double intensidadPromedioT = 0.0;
			Double velocidadPromedioN = 0.0;
			Double intensidadPromedioN = 0.0;
			System.out.println("flag 2");
			if (contadorM != 0) {
				velocidadPromedioM = velocidadMañana / contadorM;
				intensidadPromedioM = intensidadMañana / contadorM;
			}
			if (contadorT != 0) {
				velocidadPromedioT = velocidadTarde / contadorT;
				intensidadPromedioT = intensidadTarde / contadorT;
			}
			if (contadorN != 0) {
				velocidadPromedioN = velocidadNoche / contadorN;
				intensidadPromedioN = intensidadNoche / contadorN;
			}
			HashMap<String, Double> resultado = new HashMap<>();
			resultado.put("velocidadPromedio", 0.0);
			resultado.put("intensidadPromedio", 0.0);
			System.out.println("flag 3");

			// Añadir a PDF
			if(contadorM !=0 || contadorT !=0 || contadorN !=0) {
				document.add(new Paragraph(" ",font));
				document.add(new Paragraph("Lugar: " + ubicacion.getNombre(), fontLugar));
			}
			if (contadorM != 0) {
				document.add(new Paragraph("Velocidad Promedio Mañana: " + velocidadPromedioM, font));
				document.add(new Paragraph("Intensidad Promedio Mañana: " + intensidadPromedioM, font));
				document.add(new Paragraph(" ",font));
			}
			if (contadorT != 0) {
				document.add(new Paragraph("Velocidad Promedio Tarde: " + velocidadPromedioT, font));
				document.add(new Paragraph("Intensidad Promedio Tarde: " + intensidadPromedioT, font));
				document.add(new Paragraph(" ",font));

			}
			if (contadorN != 0) {
				document.add(new Paragraph("Velocidad Promedio Noche: " + velocidadPromedioN, font));
				document.add(new Paragraph("Intensidad Promedio Noche: " + intensidadPromedioN, font));
				document.add(new Paragraph(" ",font));
			}
		...
		try {
			...
			BodyPart adjunto = new MimeBodyPart();
			 adjunto.setDataHandler(new DataHandler(new FileDataSource("/home/DIINF/ncallahan/proyectos/iTextHelloWorld.pdf")));
